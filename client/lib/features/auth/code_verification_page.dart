...
class _CodeVerificationPageState extends State<CodeVerificationPage> {
  ...
  Future<void> _handleVerify() async {
    ...
    if (code.length < 4) {
      setState(() => _errorText = 'Verification code must be at least 4 characters');
      return;
    }

    // Check retry limit
    if (_retryCount >= _maxRetries) {
      setState(() => _errorText = 'Too many failed attempts. Please try again later.');
      return;
    }
    ...
    try {
      ...
      if (applicationId == null || applicationId.isEmpty) {
        setState(() {
          _errorText = 'Application not found. Please restart the verification process.';
          _isLoading = false;
        });
        return;
      }
      ...
      if (orgUsername == null || orgUsername.isEmpty) {
        ...
        if (storedUsername == null || storedUsername.isEmpty) {
          setState(() {
            _errorText = 'Verification successful but organization data is missing. Please try again.';
            _isLoading = false;
          });
          return;
        }
      ...
      switch (e.statusCode) {
        case 400:
          ...
          if (_retryCount < _maxRetries) {
            errorMessage += ' (${_maxRetries - _retryCount} attempts remaining)';
          }
          break;
        case 404:
          ...
          child: Column(
            children: [
              ...
              AppInput(
                ...
                onChanged: (_) => _clearError(), // Clear error when user starts typing
              ),
